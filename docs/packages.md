...
| c | `c/` | `c-v*` | `libmcpb`, the device-side client in C99: WebSocket, dedicated and multiplexed endpoints, registration, reconnection, link events. Transport only, by design: it carries opaque bytes and embeds no JSON, JSON-RPC or MCP layer, which stay in the firmware's own language. Ports for the host (Linux, macOS, Windows), ESP-IDF and Unreal Engine 5; the ESP-IDF component `mcpb_esp` and the Unreal plugin `McpBroker`, each with a sample; a roundtrip test and an overnight soak against the Node broker. See [`c/README.md`](../c/README.md) |
